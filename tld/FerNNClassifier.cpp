...
//	
//TLD中的分类器由三个部分组成：方差分类器(用于结合积分平方图以剔除bad_boxes中方差较小的负样本)、Fern分类器、最近邻NN分类器。
//这三个分类器是级联的，也就是说每个扫描窗口必须都通过这些分类器，才能认为是含有前景目标的，才可能是当前帧的跟踪结果。
//这里的准备主要是针对Fern分类器进行的
//Fern翻译是“蕨类”，但从自己的知识构成看，更愿意理解为森林，
//这个森林有nstructs(值为10)棵树对应nstructs个基本的分类器，每个基本的分类器有structSize(值为13)个节点，
//每个节点的作用就是根据某种判定规则指定这个节点要往左子树还是往右子树进行搜索，最终搜索到leaf。


//每个节点在进行判断的时候，所依据的规则是：在当前输入的图像片patch中随机去两个点(x1,y1)和(x2,y2)，
//计算这两点的像素值f1和f2，若f1>f2则返回1，也即往右子树的路径走，反之返回0，往左子树的路径走。
//而这里对Fern分类器的准备工作就是为features[s][i]申请空间，这里的s表示scales中的尺度索引，
//取值范围是[0,scales.size()-1]之间的整数，i表示第i个节点，取值范围是[0,129]，这里的129是10*13-1得到的，对应这个森林中所有树的所有节点。
//从而features[s][i]表示当前尺度s下第i个节点存储的两个随机点的坐标，其坐标是由当前尺度下的宽高度乘以一个(0,1)之间的随机数得到的，
//在用每个节点确定路径的时候，就是用此节点的两个随机坐标在输入图像片patch中的像素值进行比较来得到0或1，
//每棵树的13个节点就得到一个长度为13的二进制编码x，这样的二进制编码取值有2.^13种可能，

//而每一个x又对应一个后验概率post=pNum/nNum，其中pNum和nNum分别对应正负样本的个数。
//由于森林(Fern)中有10棵树，从而可以得到10个后验概率，将10个后验概率平均后若结果大于阈值thr_nn(初始经验值是0.65，从myParam.yml中获取，后面会更新)，则认为该图像片patch含有跟踪目标。
//由于x有2.^13种可能，所以后验概率post、正样本数目pNum和负样本数目nNum的容器规模也是2.^13，在本函数的最后也就是为其申请空间。
...
void FerNNClassifier::prepare(const vector<Size>& scales)
{
	...
	{
		posteriors.push_back(vector<float>(pow(2.0,structSize), 0));  //后验概率post
		pCounter.push_back(vector<int>(pow(2.0,structSize), 0));  //正样本数目pNum
		nCounter.push_back(vector<int>(pow(2.0,structSize), 0));  //负样本数目nNum
	}
}
