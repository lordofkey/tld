...
void TLD::init(const Mat& frame1,const Rect& box,FILE* bb_file){
	...
	//Preparation
	//allocation
	iisum.create(frame1.rows+1,frame1.cols+1,CV_32F); //积分图
	iisqsum.create(frame1.rows+1,frame1.cols+1,CV_64F); //平方积分图
...
//将img其缩放为patch_size*patch_size(15*15)大小然后计算均值与方差
//将每个元素的像素值减去均值使得得到的patern的均值为0。
...
//主要功能是获取当前帧中的所有扫描窗口，这些窗口有21个尺度，缩放系数是1.2
//在每个尺度下，扫描窗口的步长为宽高的10%(SHIFT=0.1)
//这个grid的每个元素包含6个属性：当前扫描窗口左上角的x坐标、y坐标、宽度w、高度h、与初始跟踪区域box的重叠率overlap、当前扫描窗口所在的尺度sidx。
//其中重叠率的定义是：两个窗口的交集/两个窗口的并集。
//与此同时，如果当前窗口的宽度和高度不小于最小窗口的阈值min_win(固定值15，从myParam.yml中获取)
//就将这个尺寸放到scales容器中，由于有min_win的限制，所以某些特别消的尺度下的扫描窗口尺寸就不会放到scales中，故该容器长度小于等于21。
